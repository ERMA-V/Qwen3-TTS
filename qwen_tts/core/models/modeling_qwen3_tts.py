...
class Qwen3TTSTalkerCodePredictorModelForConditionalGeneration(Qwen3TTSPreTrainedModel, GenerationMixin):
    ...
    def generate_fast(
        ...
        for step in range(num_codebooks):
            logits = self.lm_head[step](hidden_states[:, -1, :])
...
class Qwen3TTSForConditionalGeneration(Qwen3TTSPreTrainedModel, GenerationMixin):
    ...
    def stream_generate_pcm(
        ...
        Args:
            ...
            emit_every_frames: Emit PCM chunk every N codec frames after the first chunk
            decode_window_frames: Window size for decoding. The first chunk is held until
                this many frames are available so the decoder has full context.
        ...
        for _ in range(max_frames):
            ...
            frames_since_emit += 1
            first_emit = total_frames_emitted == 0
            # Hold the first decode until the decoder has a full context window.
            # Short windows (and left-zero-padding to decode_window_frames) distort
            # the start of the utterance.
            if first_emit:
                if len(codes_buffer) < decode_window_frames:
                    continue
            elif frames_since_emit < emit_every_frames:
                continue
            frames_since_emit = 0
            ...
            if first_emit:
                # First chunk is the full primed window so no audio is dropped.
                chunk = wav
            else:
